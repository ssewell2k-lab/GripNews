-- gamegrip_auth_schema.sql — MySQL schema for GameGrip Auth v0.6.1

CREATE DATABASE IF NOT EXISTS gamegrip
...
-- Refresh tokens table (for revocation & session management)
...
-- API keys table (for developer/SDK access)
...
-- OAuth connections (for future Google/GitHub login)
...
-- Cleanup job helper (run periodically)
